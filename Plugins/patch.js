  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`𝐓𝐑𝐄𝐗 𝐌𝐃 UPDATE WAS SUCCESSFULL ✅`");
      } else {
    ...
    try {
      ...
      const designs = [
        async () => {
          ...
          const captionText = `TREX-MD 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\n𝐓𝐑𝐄𝐗 𝐌𝐃`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*𝐓𝐑𝐄𝐗 𝐌𝐃 ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "TREX-MD Running Since",
        body: message,
        footerText: "𝐓𝐑𝐄𝐗 𝐌𝐃© 2024",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("𝐓𝐑𝐄𝐗 𝐌𝐃_FANCY_TEXT")
        ) {
